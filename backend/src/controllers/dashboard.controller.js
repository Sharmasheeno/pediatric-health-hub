    ...
    try {
        ...
        if (role === 'PARENT') {
            ...
            const rawAppts = await prisma.appointment.findMany({
                where: { parentId: req.user.profile.id, createdAt: { gte: sixMonthsAgo } },
            ...
            const rawAppts = await prisma.appointment.findMany({
                where: { doctorId: req.user.profile.id, createdAt: { gte: sixMonthsAgo } },
                select: { createdAt: true }
            });
            ...
            stats = {
                ...
                title3: "Pending Tasks", count3: await prisma.appointment.count({ where: { doctorId: req.user.profile.id, status: 'PENDING' } }),
                charts: charts
            };
        } else if (role === 'FACILITY') {
             // Real Analytics: Facility wide analytics
             const docCount = await prisma.doctorProfile.count({ where: { facilityId: req.user.profile.id } });
             
             // Find all docs in this facility
             const doctors = await prisma.doctorProfile.findMany({ where: { facilityId: req.user.profile.id }, select: { id: true }});
