...
export const DoctorInbox = () => {
    ...
    return (
        <div className="max-w-[1200px] w-full mx-auto space-y-6 animate-fade-in font-sans">
            {/* Header */}
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 sm:p-8 flex items-center gap-5 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-72 h-72 bg-blue-500/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/4 pointer-events-none" />
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="flex flex-col md:flex-row h-[520px]">
                    ...
                    <div className="w-full md:w-1/3 border-b md:border-b-0 md:border-r border-slate-100 bg-slate-50 flex flex-col">
                        <div className="p-4 border-b border-slate-200 bg-slate-100/80">
                        ...
                        <div className="flex-1 overflow-y-auto">
                            {loadingContacts ? (
                                <div className="p-6 text-center text-slate-400 text-xs font-bold uppercase tracking-widest animate-pulse">Loading contacts...</div>
                            ) : contacts?.length > 0 ? (
                                contacts.map(c => (
                                    <button
                                        key={c.id}
                                        onClick={() => setActiveContact(c)}
                                        className={`w-full text-left px-5 py-4 border-b border-slate-100 transition-all ${activeContact?.id === c.id ? 'bg-blue-50 border-l-4 border-l-blue-600' : 'hover:bg-white border-l-4 border-l-transparent'}`}
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider mt-0.5">{c.role}</div>
                                            </div>
                                        </div>
                                    </button>
                                ))
                        ...
                        {activeContact ? (
                            <>
                                ...
                                <div className="px-5 py-3 border-b border-slate-100 bg-white flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-bold text-sm text-slate-800">{activeContact.name}</div>
                                ...
                                {/* Messages */}
                                <div className="flex-1 overflow-y-auto flex flex-col gap-3 p-5 bg-slate-50/50">
                                    {(messages || []).length === 0 ? (
                                            ...
                                            return (
                                                <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                    <div className={`p-3 px-4 max-w-[75%] rounded-2xl text-sm font-medium shadow-sm ${isMe
                                                        ...
                                                        {m.content}
                                                        <div className={`text-[10px] mt-1.5 ${isMe ? 'text-blue-200' : 'text-slate-400'}`}>
                                ...
                                <div className="p-4 border-t border-slate-100 bg-white">
                                    <form onSubmit={handleSend} className="flex gap-2">
                                        <input
                                            ...
                                            className="flex-1 px-4 py-3 border-2 border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all"
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 bg-slate-50/30">
                                ...
                                <span className="font-bold text-lg text-slate-500 tracking-tight">Select a conversation</span>
