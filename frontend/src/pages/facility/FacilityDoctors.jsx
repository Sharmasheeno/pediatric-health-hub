        ...
        <div className="w-full space-y-6 animate-fade-in font-sans">
            ...
            <Card className="shadow-sm">
                <div className="p-4 border-b border-slate-100 flex items-center bg-slate-50 gap-4">
                    <div className="relative flex-1 max-w-md">
                        <input 
                            ...
                            className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-emerald-500 shadow-inner" 
                        />
                ...
                <CardContent className="p-0">
                    {isLoading ? (
                        <div className="p-12 text-center text-slate-400 font-bold uppercase tracking-widest animate-pulse">Syncing Facility Roster...</div>
                    ) : (
                                ...
                                <tbody>
                                    {displayDoctors.map(doctor => (
                                        <tr key={doctor.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="p-4">
                                                <div className="text-sm font-semibold text-slate-700">{doctor.licenseNumber}</div>
                                                <div className="text-xs text-slate-400 mt-1">{doctor.user?.email || 'N/A'}</div>
                                            </td>
                                            <td className="p-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <button onClick={() => { setFormData({ firstName: doctor.firstName, lastName: doctor.lastName, licenseNumber: doctor.licenseNumber, specialization: doctor.specialization }); setEditModal(doctor); }} className="text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 transition-colors p-2 rounded-lg"><Edit3 size={16} strokeWidth={2.5}/></button>
                                    ...
                                    {displayDoctors.length === 0 && (
                                        <tr>
                                            <td colSpan="4" className="p-8 text-center text-slate-400 font-bold uppercase tracking-widest">No Staff Found</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); createMutation.mutate(formData); }} className="p-8 space-y-4">
                            <div className="flex gap-4">
                                <div className="space-y-1 flex-1">
                                    ...
                                    <input required value={formData.email || ''} onChange={e=>setFormData({...formData, email: e.target.value})} type="email" placeholder="dr@example.com" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                                </div>
                                <div className="space-y-1 flex-1">
                                    <label className="text-[10px] font-black uppercase text-slate-400">Temporary Password</label>
                                    <input required value={formData.password || ''} onChange={e=>setFormData({...formData, password: e.target.value})} type="password" minLength="6" placeholder="Min 6 characters" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                                </div>
                            </div>
                            <div className="flex gap-4">
                                <div className="space-y-1 flex-1">
                                    <label className="text-[10px] font-black uppercase text-slate-400">First Name</label>
                                    <input required value={formData.firstName} onChange={e=>setFormData({...formData, firstName: e.target.value})} type="text" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                                </div>
                                <div className="space-y-1 flex-1">
                                    <label className="text-[10px] font-black uppercase text-slate-400">Last Name</label>
                                    <input required value={formData.lastName} onChange={e=>setFormData({...formData, lastName: e.target.value})} type="text" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                                </div>
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase text-slate-400">License Number</label>
                                <input required value={formData.licenseNumber} onChange={e=>setFormData({...formData, licenseNumber: e.target.value})} type="text" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase text-slate-400">Specialization</label>
                                <input required value={formData.specialization} onChange={e=>setFormData({...formData, specialization: e.target.value})} type="text" placeholder="e.g. Pediatric Pulmonology" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                            </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-fade-in">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden relative text-center">
                        <div className="p-8 pb-0">
                            ...
                            <h2 className="text-2xl font-black text-slate-800 tracking-tight">Remove Doctor</h2>
                            <p className="text-slate-500 font-medium text-sm mt-3">You are about to remove Dr. {deleteModal.lastName} from your facility.</p>
                        </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-fade-in">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden relative">
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); updateMutation.mutate({ id: editModal.id, data: { firstName: formData.firstName, lastName: formData.lastName, licenseNumber: formData.licenseNumber, specialization: formData.specialization } }); }} className="p-8 space-y-4">
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase text-slate-400">Specialization</label>
                                <input required value={formData.specialization} onChange={e=>setFormData({...formData, specialization: e.target.value})} type="text" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                            </div>
