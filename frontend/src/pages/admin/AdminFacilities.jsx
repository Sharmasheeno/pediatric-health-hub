        ...
        <div className="w-full space-y-6 animate-fade-in font-sans">
            ...
            <Card className="shadow-sm">
                ...
                <CardContent className="p-0">
                    {isLoading ? (
                        <div className="p-12 text-center text-slate-400 font-bold uppercase tracking-widest animate-pulse">Syncing Facility Database...</div>
                    ) : (
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {displayFacilities.map(fac => (
                                        <tr key={fac.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                            <td className="p-4">
                                                ...
                                                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">ID: {fac.id.slice(0,8)}...</div>
                                            </td>
                                            <td className="p-4">
                                                <div className="text-xs font-semibold text-slate-700">{fac.phoneNumber}</div>
                                                <div className="text-xs text-slate-400 mt-0.5">{fac.user?.email || 'N/A'}</div>
                                            </td>
                                            <td className="p-4">
                                                <div className="text-xs font-semibold text-slate-500 truncate max-w-xs">{fac.address}</div>
                                            </td>
                                            <td className="p-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <button onClick={() => { setFormData({ name: fac.name, address: fac.address, phoneNumber: fac.phoneNumber }); setEditModal(fac); }} className="text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 transition-colors p-2 rounded-lg">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                                    </button>
                                                    <button onClick={() => setDeleteModal(fac)} className="text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors p-2 rounded-lg"><Trash2 size={16} strokeWidth={2.5}/></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                                    {displayFacilities.length === 0 && (
                                        <tr>
                                            <td colSpan="4" className="p-8 text-center text-slate-400 font-bold uppercase tracking-widest">No Facilities Found</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); createMutation.mutate(formData); }} className="p-8 space-y-4">
                            <div className="flex gap-4">
                                <div className="space-y-1 flex-1">
                                    ...
                                    <input required value={formData.email || ''} onChange={e=>setFormData({...formData, email: e.target.value})} type="email" placeholder="clinic@example.com" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                                </div>
                                <div className="space-y-1 flex-1">
                            ...
                            <div className="space-y-1">
                                ...
                                <input required value={formData.name} onChange={e=>setFormData({...formData, name: e.target.value})} type="text" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase text-slate-400">Physical Address</label>
                                <input required value={formData.address} onChange={e=>setFormData({...formData, address: e.target.value})} type="text" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase text-slate-400">Contact Number</label>
                                <input required value={formData.phoneNumber} onChange={e=>setFormData({...formData, phoneNumber: e.target.value})} type="text" className="w-full border-2 border-slate-100 rounded-lg px-4 py-2.5 focus:outline-none focus:border-emerald-500 text-sm font-bold text-slate-700"/>
                            </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-fade-in">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden relative text-center">
                        <div className="p-8 pb-0">
                            ...
                            <h2 className="text-2xl font-black text-slate-800 tracking-tight">Archive Facility</h2>
                            <p className="text-slate-500 font-medium text-sm mt-3">You are about to archive the {deleteModal.name} registry record.</p>
                        </div>
