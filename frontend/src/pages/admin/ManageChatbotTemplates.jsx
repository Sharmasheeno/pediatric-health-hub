...
export const ManageChatbotTemplates = () => {
    ...
    if (user?.role !== 'ADMIN') {
        return <div className="p-20 text-center text-red-500 font-bold text-2xl uppercase tracking-widest animate-pulse">403 FORBIDDEN</div>;
    }
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6">
                ...
                <CardContent>
                    <form onSubmit={submitForm} className="space-y-4">
                        <div className="space-y-1">
                            <label className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Trigger Keywords</label>
                            <input
                                type="text"
                                ...
                                className="w-full border border-slate-300 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-500"
                            />
                        </div>
                        <div className="space-y-1">
                            <label className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Templated Response</label>
                            <textarea
                                rows={4}
                                value={form.response}
            ...
            <Card className="border-slate-200 shadow-sm">
                <CardHeader>
                    <CardTitle className="text-base text-slate-800">Existing Templates</CardTitle>
                </CardHeader>
                <CardContent>
                    {isLoading ? (
                        <div className="py-10 text-center text-slate-500">Loading templates...</div>
                    ) : templates.length === 0 ? (
                        <div className="py-10 text-center text-slate-500">No templates configured yet.</div>
                    ) : (
                        <div className="space-y-3">
                            {templates.map((template) => (
                                <div key={template.id} className="border border-slate-200 rounded-xl p-4 bg-white">
                                    <div className="text-xs uppercase tracking-wide font-semibold text-slate-500 mb-1">Keywords</div>
                                    <div className="font-semibold text-slate-800">{template.triggerKeyword}</div>
                                    <div className="text-xs uppercase tracking-wide font-semibold text-slate-500 mt-3 mb-1">Response</div>
                                    <div className="text-sm text-slate-700 whitespace-pre-wrap">{template.response}</div>
