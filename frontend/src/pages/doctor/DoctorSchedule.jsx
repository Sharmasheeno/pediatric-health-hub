        ...
        <div className="w-full space-y-6 animate-fade-in font-sans">
            ...
            <Card className="shadow-sm">
                <CardContent className="p-0">
                    {isLoading ? (
                        <div className="p-12 text-center text-[--text-muted] font-bold uppercase tracking-widest animate-pulse">Syncing Scheduling Matrix...</div>
                    ) : (
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {schedule?.map(appointment => {
                                        ...
                                        return (
                                            <tr key={appointment.id} className="border-b border-[--border] hover:bg-[--surface-soft] transition-colors">
                                                <td className="p-4">
                                                    <div className="font-bold text-[--text-primary] flex items-center gap-2">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="text-[10px] font-bold text-[--text-muted] uppercase tracking-widest mt-0.5">ID: {appointment.id?.slice(0,6)} • {appointment.reason || 'General Routine'}</div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="p-4">
                                                    <div className="text-sm font-bold text-[--text-primary] flex items-center gap-1.5"><Clock size={16} className="text-[--text-muted]"/> {new Date(appointment.scheduledAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                                                    <div className="text-xs font-semibold tracking-wide text-[--text-secondary]">{new Date(appointment.scheduledAt).toLocaleDateString()}</div>
                                                </td>
                                                <td className="p-4">
                                    ...
                                    {schedule?.length === 0 && (
                                        <tr>
                                            <td colSpan="5" className="p-12 text-center">
                                                <div className="w-16 h-16 bg-[--surface-soft] rounded-full flex items-center justify-center mx-auto mb-4 text-slate-300">
                                                    <Calendar size={32} />
                                                </div>
                                                <h3 className="text-lg font-black text-[--text-primary] tracking-tight">No Appointments</h3>
                                                <p className="text-sm text-[--text-secondary] font-medium mt-1">Your schedule matrix is completely clear.</p>
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
